...
class OKInterpolator:
    ...
    def do_interpolation(self):
        ...
        bhtinv = np.matmul(bht, inv_part)

        Rb = self.basis - np.matmul(bhtinv, cht.transpose())
        beta = np.matmul(bhtinv, bht.transpose())
        betainv = np.linalg.inv(beta)
        beta = np.matmul(np.matmul(betainv, bhtinv), hobs)

        mu = mu + np.matmul(Rb.transpose(), beta)

        p = np.matmul(inv_part, cht.transpose())
        p = np.matmul(cht, p)
        p = self.cov - p

        p = p + np.matmul(np.matmul(Rb.transpose(), betainv), Rb)
